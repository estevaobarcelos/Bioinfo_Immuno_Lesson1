...
stopifnot(all(sample_info$geo_accession %in% colnames(counts_raw)))
...
stopifnot(identical(rownames(sample_info), colnames(counts_raw)))  # sanity check


## ---- 3. Gene annotation -------------------------------------------------##
# counts_raw rows are Entrez GeneIDs; annot maps GeneID -> Symbol/description
head(annot)
gene_map <- annot %>% select(GeneID, Symbol, Description) %>%
  mutate(GeneID = as.character(GeneID))

stopifnot(all(rownames(counts_raw) %in% gene_map$GeneID))
...
EnhancedVolcano(res_df,
                ...
                title = "LPS vs baseline (all subjects, paired design)",
                subtitle = NULL)
...
rnk_export <- res_for_rank %>%
  select(Symbol, stat) %>%
  arrange(desc(stat))
write_tsv(rnk_export, "LPS_vs_baseline.rnk", col_names = FALSE)
...
EnhancedVolcano(baseline_hl$res_df,
                lab = baseline_hl$res_df$Symbol,
                x = "log2FoldChange", y = "padj",
                pCutoff = 0.05, FCcutoff = 1,
                title = "High vs Low responders — BASELINE",
                subtitle = NULL)

EnhancedVolcano(lps_hl$res_df,
                lab = lps_hl$res_df$Symbol,
                x = "log2FoldChange", y = "padj",
                pCutoff = 0.05, FCcutoff = 1,
                title = "High vs Low responders — AFTER LPS",
                subtitle = NULL)
